...
f = c(1/15,1/5) #5 to 10 days #1/10 #duration of E is approximately 10 days, Collins
mu_IS = 3 # increased death rate due to untreated malaria, 3 times higher chance
gamma = c(1/30,1/15) #1/(20) #recovery rate for both natural recovery and treatment
rep = .7 #c(.2,.8) # .70 # treatment+report coverage @@@@@ this can't be varied since its effect is very significant on the incidence and prevalence
...
a = c(1,3) #human feeding rate
b = c(.006,.01)#.006 #.01, #probability of disease transmission per bite for humans
c_ = c(.02,.08) #.02 #.03 #probability a mosquito becomes infected after biting an infected human
#seasonality
...
for(i in 1:no.sim){
  ...
  prevV[i] <- tail(prev2_mnth,2)[1]
  incV[i] <- tail(inc2_mnth,2)[1] ######Can't take last point since it maynot be a full month!!!!!!!!!!!!!!!!###########
}

plot(prevV,incV, main="Monthly incidence vs prevalence \n Using various combination of parameters")
